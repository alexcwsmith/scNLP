...
def countLitResults(cluster, rettype='full'):
    ...
    geneList = []
    countList = []
    directory = os.path.join(resultDirectory, cluster + '_' + comparison + '_papers/')
    if rettype=='full':
    ...
    countDf.to_excel(os.path.join(directory, setName + '_RetreivedLitResults_Manifest.xlsx'))
    IDdf.to_excel(os.path.join(directory, setName + '_RetreivedPMCIDs_Manifest.xlsx'))
    return(IDdf, countDf)


def updateManifest(newClusters, rettype='full', maniDf=None, IDs=False):
    """
    ...
    newClusters: string
    rettype (optional): string, 'full' or 'abstract'
    maniDf (optional): pandas DataFrame object to update
    ...
    for cluster in newClusters:
        ...
        if not IDs:
            ...
            maniDf.to_excel(os.path.join(resultDirectory, 'CombinedGeneManifest_' + comparison + '.xlsx'))
        elif IDs:
            maniDf = pd.concat([maniDf, IDdf], axis=0)
            maniDf = maniDf.sort_values(by='cluster', ascending=True)
            maniDf = maniDf.drop_duplicates(keep='first')
            maniDf.to_excel(os.path.join(resultDirectory, 'CombinedPMCIDManifest_' + comparison + '.xlsx'))
    return(maniDf)
...
def copyLit(genes, rettype='full', copyData=False):
    ...
    """
    if copyData == True:
        maniDf = pd.read_excel(os.path.join(resultDirectory, 'CombinedGeneManifest_' + comparison + '.xlsx'), index_col=0)
 #   maniDf = pd.read_excel(os.path.join(resultDirectory, 'CombinedGeneManifest_MarkerGenes.xlsx'), index_col=0)
    ...
    for pair in lz:
        if rettype=='full':
            try:
                src = os.path.join(resultDirectory, 'papers/' + pair[1] + '_' + comparison + '_papers/papers/' + pair[0] + '/CombinedFullTexts.txt')
            ...
            try:
                src = os.path.join(resultDirectory, 'papers/' + pair[1] + '_' + comparison + '_papers/abstracts/' + pair[0] + '/CombinedFullTexts.txt')
            except FileNotFoundError:
                src = os.path.join(resultDirectory, 'papers/' + pair[1] + '_' + comparison + '_papers/abstracts/' + pair[0] + '/CombinedFullTexts.txt')          
            targDir = os.path.join(targetDirectory, pair[0] + '/')
            targFile = os.path.join(targetDirectory, pair[0] + '/CombinedFullTexts.txt')
    ...
    if copyData:
        ...
        for pair in lz:
            dataDirectory = os.path.join(resultDirectory, pair[1] + '_' + comparison + '_Results')
    #        path = os.path.join(dataDirectory, pair[0])
            path = dataDirectory
            for cat in cats:
                dataPath = os.path.join(path, cat + '_' + pair[1])
                if cat=='NTs':
                    filePath = os.path.join(dataPath, pair[1] + '_' + comparison + '_model062920_' + pair[0] + '_Iteration0NER_Neurotransmitters.xlsx')
                elif cat=='Sentences':
                    filePath = os.path.join(dataPath, pair[1] + '_' + comparison + '_model062920_' + pair[0] + '_Iteration0_NER_Results_Filtered_Sentences.xlsx')                    
                else:
                    filePath = os.path.join(dataPath, pair[1] + '_' + comparison + '_model062920_' + pair[0] + '_Iteration0NER_' + cat + '.xlsx')
                dataTargPath = os.path.join(resultDirectory, cluster + '_' + comparison + '_Results/' + cat + '_' + cluster)
        ...
        with open(os.path.join(targetDirectory, pair[0] + '_CopySummary.txt'), 'w+') as f:
            if copyData==True:
                f.write(pair[0] + " copied with data from " + pair[1] + " at: " + "\n" + str(src))
            elif copyData==False:
                f.write(pair[0] + " copied text only from " + pair[1] + " at: " + "\n" + str(src))                
            f.close()
...
def retrieveAbstracts(genes, n_papers=1000, directory=paperDirectory, cluster=cluster):
    ...
    for gene in genes:
        ...
        if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < (n_papers+3):
            pm.getFullTextsGene(IDs_opioid, directory=directory, gene=gene, results=n_papers, start=0, end=None)
        if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < (n_papers+3):
            pm.getFullTextsGene(IDs_psych, directory=directory, gene=gene, results=n_papers, start=0, end=None)
        if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < (n_papers+3):
            pm.getFullTextsGene(IDs, directory=directory, gene=gene, results=n_papers, start=0, end=None)    
...
def retrieveLiterature(genes, n_papers=50, directory=paperDirectory, cluster=cluster):
    ...
    for gene in genes:
        ...
        if len(os.listdir(os.path.join(paperDirectory, 'papers/' + gene + '/'))) < (n_papers+3):
            pm.getFullTextsGene(IDs_opioid, directory=directory, gene=gene, results=n_papers, start=0, end=None)
        if len(os.listdir(os.path.join(paperDirectory, 'papers/' + gene + '/'))) < (n_papers+3):
            pm.getFullTextsGene(IDs_psych, directory=directory, gene=gene, results=n_papers, start=0, end=None)
        if len(os.listdir(os.path.join(paperDirectory, 'papers/' + gene + '/'))) < (n_papers+3):
...
def _multiRetrieveLiterature(gene):
    ...
    IDdf.to_excel(os.path.join(directory, cluster + '_' + str(gene) + '_PMCIDlist.xlsx'))
...
def _multiRetrieveAbstracts(gene):
    ...
    if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < n_papers:
        pm.getAbstractsGene(IDs_opioid, directory=directory, gene=gene, results=n_papers, start=0, end=None)
    if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < n_papers:
        pm.getAbstractsGene(IDs_psych, directory=directory, gene=gene, results=n_papers, start=0, end=None)
    if len(os.listdir(os.path.join(paperDirectory, 'abstracts/' + gene + '/'))) < n_papers:
        pm.getAbstractsGene(IDs, directory=directory, gene=gene, results=n_papers, start=0, end=None)    
    geneIDlist.extend(IDs)
    IDdf = pd.DataFrame(geneIDlist)
    IDdf['gene'] = gene
    IDdf['Cluster'] = cluster
...
def combineLiterature(paperDirectory, overwrite=False, minSize=1e4):
    ...
    for item in dirs:
        ...
        if os.path.isdir(fullpath):
            ...
            elif os.path.exists(fullpath + 'CombinedFullTexts.txt') and overwrite==True:
                pm.combineTextFiles(fullpath)
            if os.stat(os.path.join(fullpath, 'CombinedFullTexts.txt')).st_size < minSize:
...
def multiProcessLit(genes, directory, rettype='full', processes=13):
    ...
    """
    resultDirs = ['Sentences', 'Categories', 'Functions', 'Regions', 'CellTypes', 'NTs', 'Physio']
    for item in resultDirs:
        if not os.path.exists(os.path.join(clusterDirectory, item + '_' + cluster + '/')):
...
def calcEntityFrequency(directory, name, cluster):
    ...
    freqDf.to_excel(os.path.join(clusterDirectory, name + '_' + cluster + '_Frequency.xlsx'))
...
def reformatDf(df, name, cluster):
    ...
    series.to_excel(os.path.join(resultDirectory, name + '_cat_model062920_' + cluster + '.xlsx'))
    return(series)

def runNLP(cluster, n_genes=25, rettype='full', update=False, copy=True, newClusters=None, dlProcesses=3, nlpProcesses=13):
    """
    ...
    t = str(getCurrentTime())
    print("Start Retrieving Literature at: " + t + " with " + str(dlProcesses) + " nodes")
    if rettype=='full':
    ...
    t = str(getCurrentTime())
    print("Finished Retrieving Literature for " + str(len(incGenes)) + " genes at: " + t + " with " + str(dlProcesses) + " nodes")
    for n in nullGenes:
        genes.remove(n)
    if copy==True:
  ...
  #  directory = paperDirectory
    ...
    if rettype=='full':
        ...
        multiProcessLit(genes, paperDirectory, rettype='abstract', processes=nlpProcesses)
    t = str(getCurrentTime())
...
def concatFrequencies(clusters, category='Functions', save=False):
 ...
 #   resultDirectory = '/home/smith/Smith_Scripts/NLP_GeneExpression/spaCy_model062920/Results/UpOC_Abstract_Results'
    ...
    for cluster in clusters:
        directory = os.path.join(resultDirectory, cluster + '_' + comparison + '_Results/')
        df = pd.read_excel(os.path.join(directory, category + '_' + cluster + '_Frequency.xlsx'), index_col=0)
        cat = pd.concat([cat, df])
    cat = cat.sort_values(by='Occurances', ascending=False)
    if save:
        cat.to_csv(os.path.join(resultDirectory, 'Concat_' + category + '_unfiltered.csv'))
    return(cat)
...
#    resultDirectory = '/home/smith/Smith_Scripts/NLP_GeneExpression/spaCy_model062920/Results/UpOC_Abstract_Results'
    ...
    df.to_excel(os.path.join(resultDirectory, 'Combined_Clusters_' + category + '_' + comparison + '_Frequency.xlsx'))
    return(df)


def findUpregulatedEntities(cluster, category='Functions', min_count=100, alpha=.05, min_ratio=1.5):
    """
    ...
    tot = pd.read_excel(os.path.join(resultDirectory, 'Combined_Clusters_' + category + '_' + comparison + '_Frequency.xlsx'), index_col=0)
    df = pd.read_excel(os.path.join(clusterDirectory, category + '_' + cluster + '_Frequency.xlsx'), index_col=0)
   ...
   #         ratio = float(df.loc[df['word']==word]['TF-IDF'])/float(tot.loc[tot['word']==word]['TF-IDF'])
            ratio = float(df.loc[df['word']==word]['Occurances'])/float(tot.loc[tot['word']==word]['Combined Occurances'])
