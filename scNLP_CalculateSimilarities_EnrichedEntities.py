...
w2v_model = Word2Vec.load(os.path.join(w2v_dir, 'w2v_model071820_MarkerGenes_UpOC_DownOC_CombinedSentences.model'))
...
termIndex = pd.read_excel(os.path.join(resultDirectory, 'Combined_Clusters_' + category + '_' + comparison + '_Frequency.xlsx'), index_col=0)
termIndex = termIndex.sort_values(by='Combined Occurances', ascending=False)

enrichIndex = pd.read_excel('/home/smith/Smith_Scripts/NLP_GeneExpression/spaCy_model062920/Results/Combined_Clusters_Enriched_' + category + '_MarkerGenesV4_minCount300.xlsx', index_col=0)
...
enrichIndex.to_excel(resultDirectory + 'MarkerGenesV4_Combined_Enriched_' + category + '.xlsx')   
...
def calcEnrichedSimilarities(cluster, category, save=True):
    ...
    df2.columns=[cluster + ' term', cluster + '_aveSim', cluster + '_std', cluster + '_N', cluster + '_aveIndex']
    df2 = df2.sort_values(by=cluster + '_aveSim', ascending=False)
    if save:
        df2.to_excel(os.path.join(resultDirectory, cluster + '_AverageSimilarities_Enriched_' + category + modelName + '.xlsx'))        
    return(df2)
...
catDf.to_excel(os.path.join(resultDirectory, 'Combined_EnrichedSimilarities_' + category + '_w2vModel072820.xlsx'))

cat2 = pd.DataFrame()
for cluster in clusters:
    oddsList = []
    simDf = pd.read_excel(os.path.join(resultDirectory, cluster + '_AverageSimilarities_Enriched_' + category + '_w2v072820_.xlsx'), index_col=0)
    enrDf = pd.read_excel('/home/smith/Smith_Scripts/NLP_GeneExpression/spaCy_model062920/Results/MarkerGenesV4_Results/MarkerGenesV4_Combined_Enriched_' + category + '.xlsx', index_col=0)
    terms = simDf[cluster + ' term'].tolist()
    for term in terms:
        odds = enrDf.loc[enrDf[cluster + ' term']==term][cluster + '_oddsratio'].values[0]
...
cat2.to_excel(os.path.join(resultDirectory, 'MarkerGenesV4_Similarities_OddsRatios_Combined_' + category + '.xlsx'))
